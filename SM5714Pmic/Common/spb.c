...
--*/
{
	...
	if (length > DEFAULT_SPB_BUFFER_SIZE)
	{
		...
		{
			Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error allocating memory for Spb write - %!STATUS!", status);
			goto exit;
		}
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error writing to Spb - %!STATUS!", status);
		goto exit;
	}
...
--*/
{
	...
	{
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "I2CWRITE (Update bit): LENGTH=%lu", Length);
...
--*/
{
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "WdfMemoryCreatePreallocated failed creating memory for sequence" "status:%!STATUS!", status);
		goto exit;
	}
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Failed sending SPB Sequence IOCTL bytes:%lu status:%!STATUS!", (ULONG)bytes, status);
		goto exit;
	}
...
--*/
{
...
#if I2C_VERBOSE_LOGGING
	{
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "I2CWRITE (Write reg) : LENGTH=%hu", SendLength);
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "SpbSequence failed sending a sequence " "status:%!STATUS!", status);
		goto exit;
	}
...
#if I2C_VERBOSE_LOGGING
	{
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "I2CREAD (Read reg)  : LENGTH=%hu", Length);
		for (ULONG j = 0; j < Length; j++)
		{
			UCHAR byte = ((PUCHAR)Data)[j];
			DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, " %02hhX", byte);
...
--*/
{
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error setting address pointer for Spb read - %!STATUS!", status);
		goto exit;
	}
	...
	if (Length > DEFAULT_SPB_BUFFER_SIZE)
	{
		...
		if (!NT_SUCCESS(status))
		{
			Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error allocating memory for Spb read - %!STATUS!", status);
			goto exit;
		}
	...
	if (!NT_SUCCESS(status) ||
		bytesRead != Length)
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error reading from Spb - %!STATUS!", status);
		goto exit;
	}
...
--*/
{
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error creating IoTarget object - %!STATUS!", status);
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error creating Spb resource hub path string - %!STATUS!", status);
		goto exit;
	}
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error opening Spb target for communication - %!STATUS!", status);
		goto exit;
	}
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error allocating default memory for Spb write - %!STATUS!", status);
		goto exit;
	}
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error allocating default memory for Spb read - %!STATUS!", status);
		goto exit;
	}
	...
	if (!NT_SUCCESS(status))
	{
		Print(DEBUG_LEVEL_ERROR, DBG_IOCTL, "Error creating Spb Waitlock - %!STATUS!", status);
		goto exit;
	}
